 ...
 * @return const CAN_TxHeader_t* 电机CAN控制消息头指针
 */
const CAN_TxHeader_t *Motor::getMotorControlHeader() const
{
...
{
    if (CAN_GET_STD_ID(rxMessage.header) != m_motorFeedbackMessageID) return false;
      ...
      m_gearboxRatio(gearboxRatio)
{
    ...
    CAN_SET_STD_ID(m_motorControlHeader, m_motorControlMessageID);
}
